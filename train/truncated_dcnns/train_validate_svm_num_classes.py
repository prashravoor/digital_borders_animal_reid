...
def train_svm_for_layer(client, datasetName, modelName, layer, outfile, imageNames, num_classes):
    ...
    for trType in [None, 'pca']: # trType in [None, 'pca', 'logm']:
        ...
        ids = set([x.animalId for x in records])
        ids = np.random.choice(list(ids), num_classes, replace=False)
        ...
        obs = np.array([normalize_activations(x.feature) for x in records if x.animalId in ids])
        dbEnd = time.time()
        #obs = np.array([normalize_activations(x) for x in obs])
        target = [x.animalId for x in records if x.animalId in ids]
        del records
...
def find_best_svm_model(dsName, modelNames, num_classes):
    ...
    for modelName in modelNames:
        ...
        for layer in layerNames:
            ...
            acc, trType, kernelType,times = train_svm_for_layer(client, dsName, modelName, layer, outfile, imageNames, num_classes)
            count += 1
            sum_times += times
...
if __name__ == '__main__':
    ...
    for ds in datasets:
        ...
        acc,layer,model,transform,kernel,avg_time = find_best_svm_model(ds, modelNames, num_classes)
