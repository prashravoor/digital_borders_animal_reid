...
def showSixImagesByIndex(indices, nameslist, targetlist, qidx, valnames, valtgt, SENT_CLASS, rank=5):
    m = len(qidx)
    ...
    for i in range(m):
        ...
        for j in range(1,n):
            ...
            if files[j-1] == str(SENT_CLASS):
                image = cv2.rectangle(image, (thickness,thickness), (w-thickness,h-thickness), color=(255,0,0), thickness=thickness)
            elif crct[j-1]:
...
def displayQualResult(distmat, mean, std, y_train, y_val, x_train_names, x_val_names):
    m, n = distmat.shape
    ranks = [1, 3, 5, 10, 20, 50]
    SENT_CLASS = 999

    threshold_dist = mean + (2 * std)
    print('Threshold: {}'.format(threshold_dist))
    # Append dummy class at end of y_train
    x_train_names = np.append(x_train_names, SENT_CLASS)
    y_train = np.append(y_train, SENT_CLASS)
    y_gal_unique = set(y_gal)
    ...
    resindices = []
    resqidx = []
    #ids = np.random.choice(y_val, numimages, replace=False)
    ids = [x for x in y_val if not x in y_train] # Open set ids only
    print('Testing over total {} ids'.format(ids))
    if len(ids) < numimages:
        ids.extend(np.random.choice([x for x in y_val if not x in ids], numimages - len(ids), replace=False))
    id_map = dict()
    ...
    for x in ids:
        ...
        openreid = False
        if not org_class in y_gal_unique:
            org_class = SENT_CLASS # Query Id does not appear in gallery
            openreid = True
    ...
    showSixImagesByIndex(resindices, x_train_names, y_train, resqidx, x_val_names, y_val, SENT_CLASS)
...
if __name__ == '__main__':
    ...
    print('Missing id list: {}'.format([x for x in y_qry if x not in y_gal]))
    x_gal, x_qry = extractFeatures(x_gal_names, x_qry_names, model)
    distmat = pdist(x_qry, x_gal)
    
    displayQualResult(distmat, mean, std, y_gal, y_qry, x_gal_names, x_qry_names)        
